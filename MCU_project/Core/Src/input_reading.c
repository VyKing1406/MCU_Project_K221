...
int flag_auto = 0;
uint16_t Pin_input[4] = {Mode_Button_Pin, Add_Button_Pin, Confirm_Button_Pin, Perdes_Button_Pin };
void button_reading() {
		...
		if ((debounceButtonBuffer0[i] == debounceButtonBuffer1[i])
			...
			if (debounceButtonBuffer3[i] != debounceButtonBuffer2[i]) {
				...
				if (debounceButtonBuffer2[i] == BUTTON_IS_PRESSED) {//button is press less 3s
					button_process(i); //function to perform the function corresponding to each mode
